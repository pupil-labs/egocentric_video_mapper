...
class DISK_LightGlueImageMatcher(ImageMatcher):
    def __init__(self, num_features, gpu_num=None):
        ...
        if gpu_num is None:
            ...
            self.device= torch.device(f'cuda:{gpu_num}' if torch.cuda.is_available() else 'cpu')
        self.feature_extractor = kornia.feature.DISK.from_pretrained('depth').to(self.device)
    ...
    def get_correspondences(self, src_image, dst_image, src_patch_corners=None): #add dst corner, 
        ...
        with torch.inference_mode():
            ...
            features0 = self.feature_extractor(src_tensor,self.num_features,pad_if_not_divisible=True)[0]
            features1 = self.feature_extractor(dst_tensor,self.num_features,pad_if_not_divisible=True)[0]
            image_src = {
                "keypoints": features0.keypoints[None],
                "descriptors": features0.descriptors[None],
                "image_size": torch.tensor(src_tensor.shape[-2:][::-1]).view(1, 2).to(self.device),
            }
            image_dst = {
                "keypoints": features1.keypoints[None],
                "descriptors": features1.descriptors[None],
                "image_size": torch.tensor(dst_tensor.shape[-2:][::-1]).view(1, 2).to(self.device),
            }
        ...
        correspondences = {'keypoints0':kp0[idxs[:, 0]],'keypoints1':kp1[idxs[:, 1]],'confidence':scores}
        if src_patch_corners is not None:
            correspondences['keypoints0']=correspondences['keypoints0']+src_patch_corners[0]
        return correspondences
