...
class OpticFlowCalculatorBase(ABC):
    def __init__(self, video_dir):
        ...
        self.optic_flow_result = pd.DataFrame.from_dict(dict(start=[],end=[],avg_displacement_x=[], avg_displacement_y=[], angle=[]))
    ...
    def get_optic_flow(self, start_timestamp, end_timestamp): 
        timestamps_for_optic_flow = self.video.get_timestamps(start_timestamp,end_timestamp) 
        requested_optic_flow = dict(start=[],end=[],avg_displacement_x=[], avg_displacement_y=[], angle=[])
        ...
        requested_optic_flow = pd.DataFrame.from_dict(requested_optic_flow,orient='columns')
        self.optic_flow_result = pd.concat([self.optic_flow_result,requested_optic_flow]).drop_duplicates()
    ...
    def _retrieve_optic_flow(self, ts1, ts2): # check if the optic flow between two frames has already been calculated and returns it 
        optic_flow_query = self.optic_flow_result[(self.optic_flow_result['start']==ts1) & (self.optic_flow_result['end']==ts2)]
...
class OpticFlowCalculatorLK(OpticFlowCalculatorBase):
    ...
    def _create_grid_points(self): 
        ...
        point_coordinates = np.array([[xx[i,j],yy[i,j]] for i in range(xx.shape[0]) for j in range(xx.shape[1])])
    ...
    def _calculate_optical_flow_between_frames(self, first_frame, second_frame): 
        p1, status, err = cv.calcOpticalFlowPyrLK(first_frame, second_frame, self.points_to_track, None, **self.lk_params)
        p0r, status, err = cv.calcOpticalFlowPyrLK(second_frame,first_frame, p1, None, **self.lk_params)
...
class OpticFlowCalculatorFarneback(OpticFlowCalculatorBase):
    def __init__(self, video_dir):
        super().__init__(video_dir)
        self.farneback_params = dict(pyr_scale=0.5, levels=3, winsize=15, iterations=3, poly_n=5, poly_sigma=1.2, flags=0)
    
    def _calculate_optical_flow_between_frames(self, first_frame, second_frame):
        dense_optical_flow = cv.calcOpticalFlowFarneback(cv.cvtColor(first_frame, cv.COLOR_BGR2GRAY), cv.cvtColor(second_frame, cv.COLOR_BGR2GRAY), None, **self.farneback_params)
        ...
        return dict(avg_displacement_x=avg_displacement_x, avg_displacement_y=avg_displacement_y, angle=np.mean(angle))